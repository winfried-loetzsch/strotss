...
from helpers import torch_to_np, load_image_cuda, save_as_image
...
def optimize(vp_grad, effect, content, style, scale, content_weight, lr, extractor):
    ...
    # use rmsprop
    optimizer = optim.RMSprop(vp_grad.parameters(), lr=lr)
    optimizer.zero_grad()

    # extract features for content
    feat_content = extractor(content)  #

    stylized = effect(content, vp_grad())
...
class ParameterContainer(torch.nn.Module):
    def __init__(self, vp, repeat_batch_dim=1):
        super().__init__()
        self.vp = torch.nn.Parameter(vp)
    ...
    def forward(self):
        ...
        return torch.cat(self.repeat_batch_dim * [self.vp], dim=0)
...
def strotss(content_pil, style_pil, content_weight=1.0 * 16.0, device='cuda:0', space='uniform'):
    ...
    effect = WatercolorEffect().cuda().train()
    # effect.depth_enhance = IdentityEffect()
    effect.enable_checkpoints()
    vp = effect.vpd.preset_tensor(coffee_preset, torch.zeros((1, 1, 1, 1)), True).cuda()
    vp_grad = ParameterContainer(vp)
    ...
    for scale in scales:
        ...
        vp_grad = ParameterContainer(
            tensor_resample(vp_grad.vp.data, [content_full.shape[2] // scale, content_full.shape[3] // scale]))
        ...
        result = optimize(vp_grad, effect, content, style, scale, content_weight=content_weight, lr=lr,
                          extractor=extractor)
    ...
    return np_to_pil(result_image * 255.0), vp_grad()


def main():
    parser = ArgumentParser()
    parser.add_argument("--content", type=str, default="flowers_big.jpg")
    parser.add_argument("--style", type=str, default="watercolor_flowers.JPG")
    ...
    start = time()
    result, ppp = strotss(pil_resize_long_edge_to(content_pil, args.resize_to),
                          pil_resize_long_edge_to(style_pil, args.resize_to), content_weight, device, args.ospace)
    result.save(args.output)
    torch.save(ppp.detach().clone(), args.output + ".pt")

    print(f'Done in {time() - start:.3f}s')


def ttt():
    img = Image.open("flowers_big.jpg")
    img = pil_resize_long_edge_to(img, 1024)
    img = np_to_tensor(pil_to_np(img), "uniform").cuda()
    ppp = torch.load("strotss.png.pt")
    img = F.interpolate(img, (ppp.size(2), ppp.size(3)))

    effect = WatercolorEffect().cuda()
    # effect.enable_checkpoints()

    res = effect(img, ppp)
    save_as_image(res, "strotss_test.png")
